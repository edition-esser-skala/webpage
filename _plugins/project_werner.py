"""Pages for the Werner Proprium Missae project."""
...
works by Gregor Joseph Werner.

MIDI files of all works are available [in this archive](https://edition.esser-skala.at/assets/pdf/werner-proprium-missae/midi_collection.zip).
...
PDF_LINK_TEMPLATE = ("[{part_name}](https://edition.esser-skala.at/assets/"
                     "pdf/werner-proprium-missae/{work}/{file})"
                     "{{: .asset-link{cls}}}")


def add_project_werner(gh_org: Organization) -> None:
    ...
    last_tag = gh_org.get_repo("werner-proprium-missae").get_tags()[0].name

    with tempfile.TemporaryDirectory() as repo_dir:
        git.Repo.clone_from(
            "https://github.com/edition-esser-skala/werner-proprium-missae",
            repo_dir,
    ...
    with open("_pages/projects/werner-proprium-missae.md", "w", encoding="utf-8") as f:
        f.write(
            PAGE_TEMPLATE.format(
                title="Werner's Proprium Missæ",
                permalink="/projects/werner-proprium-missae/",
